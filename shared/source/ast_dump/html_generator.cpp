...
static void createHtmlDump(VisualizerContext* context, DumpInfo* info,
    const char* image, size_t image_counter);
static void writeTreeInfo(VisualizerContext* context, DumpInfo* info, size_t image_counter);
...
{
    assert(context.id_table); assert(ast_root); assert(file);
    ...
    DumpInfo info = {message, file, function, line};
...
static void createHtmlDump(VisualizerContext* context, DumpInfo* info,
    const char* image, size_t image_counter)
{
    assert(context); assert(info); assert(image);

    fprintf(context->dump_file,
        "<html>\n"
        "\t<style>\n"
...
static void writeTreeInfo(VisualizerContext* context, DumpInfo* info, size_t image_counter)
{
    assert(context); assert(info);

    fprintf(context->dump_file,
        "\t\t<h1>AST DUMP #%03zu</h1>\n"
        "\t\t<h2>Dump {%s:%d} called from %s()</h2>\n"
        "\t\t<h3>MESSAGE: %s</h3>\n",
        image_counter, info->file, info->line, info->function, info->message);
}
